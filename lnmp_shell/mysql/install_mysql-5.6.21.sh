...
if [ $machine == "x86_64" ];then
  ...
  mv mysql-5.6.21-linux-glibc2.5-x86_64/* /usr/local/webserver/mysql
else
  ...
  mv mysql-5.6.21-linux-glibc2.5-i686/* /usr/local/webserver/mysql
fi
...
/usr/local/webserver/mysql/scripts/mysql_install_db --datadir=/usr/local/webserver/mysql/data/ --basedir=/usr/local/webserver/mysql --user=mysql
chown -R mysql:mysql /usr/local/webserver/mysql/
chown -R mysql:mysql /usr/local/webserver/mysql/data/
chown -R mysql:mysql /home/logs/mysql
\cp -f /usr/local/webserver/mysql/support-files/mysql.server /etc/init.d/mysqld
sed -i 's#^basedir=$#basedir=/usr/local/webserver/mysql#' /etc/init.d/mysqld
sed -i 's#^datadir=$#datadir=/usr/local/webserver/mysql/data#' /etc/init.d/mysqld
cat > /etc/my.cnf <<END
[client]
port            = 3306
