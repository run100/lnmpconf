...
#zend
if ls -l /usr/local/webserver/ |grep "5.3.18" > /dev/null;then
...
elif ls -l /usr/local/webserver/ |grep "5.5.7" > /dev/null;then
  #mkdir -p /usr/local/webserver/php/lib/php/extensions/no-debug-non-zts-20121212/
